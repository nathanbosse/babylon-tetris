...
const VRController = ({ scene, controllerInput }) => {
  ...
  useEffect(() => {
    if (scene && !XREnabled) {
      const setupXR = async () => {
        ...
        xr.teleportation.rotationEnabled = false
  ...
  const handleRightJoystickInput = (values) => {
    // Define the dead zone and sensitivity thresholds
    const deadZone = 0.2 // Joystick movement below this value will be ignored
    const moveThreshold = 0.5 // Joystick movement must exceed this value to trigger directional input

    // Handling vertical movement with dead zone and threshold check
    if (Math.abs(values.y) > deadZone) {
      if (values.y > moveThreshold) {
        debouncedDown()
        upFired.current = false
      } else if (values.y < -moveThreshold) {
        if (!upFired.current) {
          controllerInput('up')
    ...
    // Handling horizontal movement with dead zone and threshold check
    if (Math.abs(values.x) > deadZone) {
      if (values.x < -moveThreshold) {
        debouncedLeft()
      } else if (values.x > moveThreshold) {
        debouncedRight()
      }
    }
  }
